...
function setupEventListeners() {
    ...
    if (lookPad && lookKnob) {
        ...
        const updateLook = (clientX, clientY) => {
            ...
            lookKnob.style.transform = `translate(calc(-50% + ${ndx}px), calc(-50% + ${ndy}px))`;
        };
        ...
        lookPad.addEventListener('pointerleave', () => {
            if (!lookActive) return;
            lookActive = false;
            lookPointerId = null;
            resetLook();
    ...
    document.addEventListener('pointerdown', (event) => {
        ...
        if (event.target && event.target.closest && event.target.closest('#mobileControls')) return;
    ...
    document.addEventListener('pointermove', (event) => {
        ...
        const dx = event.clientX - mobileLastX;
        const dy = event.clientY - mobileLastY;
        mobileLastX = event.clientX;
        mobileLastY = event.clientY;
    ...
    document.addEventListener('pointerup', endMobileLook, { passive: false });
    document.addEventListener('pointercancel', endMobileLook, { passive: false });
...
function animate() {
    ...
    if (gameState === 'playing') {
        ...
        const absY = Math.abs(lookY);
        if (absX > 0.01 || absY > 0.01) {
            const deadZone = 0.08;
            const sx = absX < deadZone ? 0 : (absX - deadZone) / (1 - deadZone) * Math.sign(lookX);
            const sy = absY < deadZone ? 0 : (absY - deadZone) / (1 - deadZone) * Math.sign(lookY);
